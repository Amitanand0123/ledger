...
export const invokeAgent = async (userId: string, resumeId: string, jobId: string, userGoal: string) => {
    ...
    let resumeText = '';
    try {
        resumeText = await extractTextFromPdf(resumeFileBuffer);
    } catch (error) {
...
export const rebuildResumeAndGeneratePdf = async (userId: string, resumeId: string, jobId: string) => {
    ...
    const companySlug = (job.company || 'Unknown_Company').replace(/\s/g, '_');
    const newFilename = `Resume_for_${companySlug}.pdf`;
...
export const rebuildResumeStandalone = async (userId: string, resumeId: string, jobDescription: string) => {
    if (!config.aiServiceUrl || !config.aiServiceApiKey) {
        throw new ApiError(503, 'AI service is not configured.');
    }
    ...
    if (!resumeDoc || !resumeDoc.latexSource) {
        throw new ApiError(400, 'Resume not found or has no LaTeX source code.');
    }

    logger.info(`Requesting AI to rebuild resume ${resumeId} (standalone)`);
    const aiResponse = await fetch(`${config.aiServiceUrl}/rebuild-resume-latex`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${config.aiServiceApiKey}` },
        body: JSON.stringify({

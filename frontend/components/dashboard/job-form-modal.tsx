      ...
      <DialogContent className="max-w-6xl max-h-[90vh] w-[95vw]">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 py-4 max-h-[calc(90vh-180px)] overflow-y-auto pr-2">
            ...
            <div className="space-y-4 rounded-md border p-4">
                ...
                <div className="space-y-1"><Label>Resume</Label><Controller name="resumeId" control={control} render={({ field }) => (<Select onValueChange={(v) => field.onChange(v === 'none' ? null : v)} value={field.value ?? 'none'}><SelectTrigger disabled={isLoadingResumes}>{isLoadingResumes ? <Loader2 className="h-4 w-4 animate-spin"/> : <SelectValue placeholder="Select a resume" />}</SelectTrigger><SelectContent><SelectItem value="none">None</SelectItem>{resumes?.map(d => <SelectItem key={d.id} value={d.id}><FileText className="inline-block mr-2 h-4 w-4"/>{d.filename}</SelectItem>)}</SelectContent></Select>)} /></div>
                <div className="space-y-1"><Label>Cover Letter</Label><Controller name="coverLetterId" control={control} render={({ field }) => (<Select onValueChange={(v) => field.onChange(v === 'none' ? null : v)} value={field.value ?? 'none'}><SelectTrigger disabled={isLoadingCoverLetters}>{isLoadingCoverLetters ? <Loader2 className="h-4 w-4 animate-spin"/> : <SelectValue placeholder="Select a cover letter" />}</SelectTrigger><SelectContent><SelectItem value="none">None</SelectItem>{coverLetters?.map(d => <SelectItem key={d.id} value={d.id}><FileText className="inline-block mr-2 h-4 w-4"/>{d.filename}</SelectItem>)}</SelectContent></Select>)} /></div>
